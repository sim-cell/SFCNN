...
writer = SummaryWriter("runs/sfcnn_base_cifar10")
...
model = SFCNN(num_classes=num_classes, block_numbers=[8, 15, 35, 8], channels=[80*2**i for i in range(4)],expand_ratio=3).to(device)
...
torch.save(model.state_dict(), 'models/sfcnn_base_cifar10_run2.pth')  
writer.close()
