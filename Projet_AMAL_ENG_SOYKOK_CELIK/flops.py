...
with open("results/flops/flops_vit_224.txt", "w") as f:
    f.write(f"{'Model':<10}{'Input Shape':<30}{'FLOPs':<20}{'Params (M)':<20}\n")
    f.write("="*70 + "\n")
    macs, params = apply_flop(model, input)
    print(f"Dimensions: {input.shape}")
...
with open("results/flops/flops_convnext_tiny.txt", "w") as f:
    f.write(f"{'Model':<10}{'Input Shape':<30}{'FLOPs':<20}{'Params (M)':<20}\n")
    f.write("="*70 + "\n")
    macs, params = apply_flop(model, input)
    print(f"Dimensions: {input.shape}")
    print(f"ConvNext Tiny FLOPs: {macs}")
    print(f"ConvNext Tiny Params: {params}")
    f.write(f"{'ConvNext Tiny':<10}{str(input.shape):<30}{macs:<20}{params:<20}\n")